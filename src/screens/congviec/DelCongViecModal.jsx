...
import { deleteByID } from '../../api/Api_CongViec';

const DelCongViecModal = ({ visible, onClose, onSearch, macv }) => {

    const base_url = React.useContext(GlobalContext).url;
    const [keyword] = useState("");
    const [trangthai] = useState([]);
    const [loaicv] = useState([]);
    const [manvth] = useState([]);
    const [ngaybd] = useState("");
    const [ngaykt] = useState("");

    const handleDelete = async () => {
        try {
            const response = await deleteByID(base_url, macv);
            if (response.resultCode) {             
                await onSearch(keyword, trangthai, loaicv, manvth, ngaybd, ngaykt);
                Alert.alert('Kết quả', `Xóa thành công công việc bảo trì với ID: ${macv}`);
...
export default DelCongViecModal;
