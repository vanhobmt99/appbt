...
import { deleteByLapKeHoachID } from '../../api/Api_LapKeHoach';

const DelLapKeHoachModal = ({ visible, onClose, onSearch, makh }) => {

    const base_url = React.useContext(GlobalContext).url;
    const [keyword] = useState(""); 
    const [nambt] = useState(0);

    const handleDelete = async () => {
        try {
            const response = await deleteByLapKeHoachID(base_url, makh);
            if (response.resultCode) {             
                await onSearch(keyword, nambt);
                Alert.alert('Kết quả', `Xóa thành công kế hoạch của thiết bị với ID: ${makh}`);
...
export default DelLapKeHoachModal;
