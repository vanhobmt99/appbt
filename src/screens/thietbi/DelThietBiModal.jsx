...
import { deleteByThietBiID } from '../../api/Api_ThietBi';

const DelThietBiModal = ({ visible, onClose, onSearch, thietBiId }) => {

    const base_url = React.useContext(GlobalContext).url;
    const [keyword] = useState("");          
    const [loaitb] = useState(0);
    const [trangthai] = useState(0);

    const handleDelete = async () => {
        try {
            const response = await deleteByThietBiID(base_url, thietBiId);
            if (response.resultCode) {             
                await onSearch(keyword, loaitb, trangthai);
                Alert.alert('Kết quả', `Xóa thành công thiết bị với ID: ${thietBiId}`);
...
export default DelThietBiModal;
